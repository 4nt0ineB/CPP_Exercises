...
{
    apply_on_entities_with_type(all_entities, type, [](const Entities& entities) {
        for(const auto& entity: entities) {
            std::cout << "Name: " << entity->name << ", Life: " << entity->life << std::endl;
        }
    });
}
...
void remove_dead_entities(std::map<std::string, Entities>& all_entities)
{
    auto remove_if_dead = [](Entities& entities) {
        entities.remove_if([](const auto& entity) { return entity->life <= 0; });
    };
